...
  在计算最终成绩时将综合考虑三个track的分数，对于没有参加某个track的队伍，将按照该track的baseline分数进行加权，从而计算总分数。
  ...
  测试阶段：任务组织者将在每晚8点至10点对当日提交的结果进行得分计算，计算结果将更新至榜单文件（CERRE-2025CCL-RankList，Excel文件）。请各位评测参与者按照"/trackN/CCLDataset_blind_test_answer_2025XXXXXXXX.json"（N表示赛道号码，XXXXXXXX表示时间，分别为月2位、日2位、时2位、分2位）的路径与命名规范提交测试结果，为考虑各队刷榜需要，各赛道可最多同时保留5份按要求命名的json文件。至测试阶段末尾，任务组织者会根据各队伍的实际情况适当提高计算得分的频率，以便各队刷榜。
