...
[2025/04/15] 更新各赛道评价标准、发布测试代码：CERRE-2025CCL/evaluation_script.py。

[2025/04/17] 设置问答板块（QA），将定期整理问题与回答更新至页面末尾。

[2025/05/05] 开始测试阶段，报名结束。盲测集、测试渠道发布。

[2025/05/17] 中午 12:00 测试阶段结束，提交终止。
...
  在计算最终成绩时将综合考虑三个track的分数，对于没有参加某个track的队伍，将按照该track的baseline分数进行加权，从而计算总分数。
  ...
  测试阶段：任务组织者将在每晚8点至10点对当日提交的结果进行得分计算，计算结果将更新至榜单文件（CERRE-2025CCL-RankList，Excel文件）。请各位评测参与者按照"/trackN/CCLDataset_blind_test_answer_2025XXXXXXXX.json"（N表示赛道号码，XXXXXXXX表示时间，分别为月2位、日2位、时2位、分2位）的路径与命名规范提交测试结果。
  
  为考虑各队刷榜需要，各赛道可最多同时保留5份按要求命名的json文件（注意，并不是指整个测试阶段只有5次答案提交机会，而是指同时只比较计算5份答案以获得最高分；每天更新的榜单中会显示评测者得分最高的文件名，评测者可自行替换答案文件，但最多同时只能保留5份）。至测试阶段末尾，任务组织者会根据各队伍的实际情况适当提高计算得分的频率，以便各队刷榜。
...
### 任务描述
  ...
  本赛道以文档为基本单位，将修辞手法按粗粒度分类为：比喻、比拟、夸张、排比、反复、设问、反问、摹状等8种粗粒度类型；同时，每类修辞从形式角度进一步细粒度分类为如下18种类型：
...
（4）针对排比修辞，不论形式如何，修辞成分都包括连接词（排比结构）和描写内容（排比内容）。
...
## 问答板块（QA）
  ...
  Q1: 关于【排比】中conjunction的内容，今年的数据中有如省略号、括号的内容，不清楚这个在成分抽取赛道中是否要求理解与识别出来。包括【反复】中也存在类似情况。

  A1: 我们在本次修辞成分抽取赛道中的评估中，conjunction里中文省略号、中文括号及括号内的内容，在评测中不影响各个队伍的评分结果。请参与成分抽取赛道的大家重点关注“中文省略号/中文括号/中文括号内容”以外的内容，以及各个成分的idx。

  Q2: 训练集和测试集为什么数量悬殊这么大？

  A2: 训练集的50个数据是精细抽样后得到的（均有标注结果、绝大多数含有修辞），而盲测集中可能存在大量的无修辞文档。其次，注意本次评测并不是测试集3万条数据，而是盲测集3万条，这样设计是为了防止部分评测者人工做出答案。在真实评测时，我们只会选择盲测集中的子集进行得分计算。

  Q3: 提交细节？

  A3: 给到各位评测者的盲测集的结构为\{"idx": int, "document": str, "sentenceList": list, "rhetoricItems": list\}。其中"idx"、"document"、"sentenceList"作为评测提供信息原则上不作修改，评测者根据各赛道的数据规范（参考数据样例、训练集），在"rhetoricItems"键对应的列表型值中填写模型的预测结果。最终将json文件按要求规范提交到本评测的测试渠道中。
