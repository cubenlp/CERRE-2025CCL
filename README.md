...
预计 [2025/05/05]～[2025/05/19] 进行测试、打榜，测试阶段开始前均可报名。
...
## 问答板块（QA）
  ...
  A2: 训练集的50个数据是精细抽样后得到的（均有标注结果、绝大多数含有修辞），而盲测集中可能存在大量的无修辞文档。其次，注意本次评测并不是测试集2万条数据，而是盲测集2万条，这样设计是为了防止部分评测者人工做出答案。在真实评测时，我们只会选择盲测集中的子集进行得分计算。
