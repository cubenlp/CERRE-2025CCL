...
def component_micro_f1_calculate(true_labels_list, pred_labels_list):
    ...
    f1: f1分数
    """
    if true_labels_list == pred_labels_list:
        return 1.0
    total_tp = 0
    total_fp = 0
    total_fn = 0
...
class AnswerEvaluator:
    ...
    def component_score_calculate(
        ...
        ref_conjunctions = [item[0] for item in reference_label_list]
        ref_tenors = [item[1] for item in reference_label_list]
        ref_vehicles = [item[2] for item in reference_label_list]

        ans_conjunctions = [item[0] for item in answer_label_list]
        ans_tenors = [item[1] for item in answer_label_list]
        ans_vehicles = [item[2] for item in answer_label_list]
